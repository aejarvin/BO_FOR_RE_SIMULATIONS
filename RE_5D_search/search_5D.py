...
def sim_fn(Tf, nAr, log_walltime, alpha, beta, batch_size=1, random_state=None):
    ...
    file1 = run_CQ_fluid_simulation(T_f=Tf[0],nAr_frac=nAr[0], 
                                    walltime=float(np.exp(log_walltime[0])), 
                                    alpha=alpha[0],
                                    beta=beta[0], 
                                    Nre = 1e10,
                                    initoutfile = 'bolfi_5D_runs/'+str(float(Tf))+'init_out.h5',
                                    CQfluidoutfile = 'bolfi_5D_runs/'+str(float(Tf))+'CQ_fluid_out.h5')
...
def get_model(true_params=None, seed_obs=None, **kwargs):
    ...
    # Define the prior distribution for the uncertain parameters
    priors = []
...
if __name__ == '__main__':
    ...
    kernel = GPy.kern.RatQuad(input_dim=5, ARD=True)
    # Constrain the kernel lenthscale to be positive and
    # power between 1e-10 and 0.03
    ...
    bolfi.acquisition_method = RandMaxVar(model=bolfi.target_model,
                                          noise_var=0, prior=ModelPrior(bolfi.model),
                                          sampler='metropolis',n_samples=200)
    ...
    # Fit the GPR
    inst = tmn.instance
    # Set Gaussian noise to 1e-10
    inst.Gaussian_noise.variance = 1e-10
    # Fix Gaussian noise
    inst.Gaussian_noise.variance.fix()
    # Randomize the GPR hyperparameters
    inst.randomize()
    # Set the initial lengthscales equal to width of the search space
        ...
        if i > 5:
            ...
            while iteration_accepted == False:
                try:
                    ...
                    # Save the bolfi result to a pickle file
                    save_bolfi(bolfi,fname='BOLFI_5D_at_'+str(10*i)+'.pkl')
                    # Refit GPR
                    inst = tmn.instance
                    inst.Gaussian_noise.variance = 1e-10
                    inst.Gaussian_noise.variance.fix()
                    ...
                    if i%2 == 0: 
                        ...
                        # log_walltime lengthscale constraint
                        kernel.lengthscale[[3]].constrain_bounded(1e-3,0.1)
                        # Argon fraction lengthscale constraint
                        kernel.lengthscale[[4]].constrain_bounded(1e-3,1.0)
                        inst.randomize()
                        inst.optimize()
                    iteration_accepted = True
                except:
