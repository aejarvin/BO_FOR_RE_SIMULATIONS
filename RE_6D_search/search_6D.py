...
. 
def sim_fn(Tf1, Tf2, tmin, nAr, alpha, beta, batch_size=1, random_state=None):
    ...
    file1 = run_CQ_fluid_simulation_vTf(Tf1=Tf1[0], Tf2=Tf2[0], t_T_f=tmin[0], nAr_frac=nAr[0], 
                                    ...
                                    beta=beta[0], 
                                    Nre = 1e10,
                                    inputfilename = 'bolfi_6D_runs/test1i.h5',
                                    outputfilename = 'bolfi_6D_runs/test1o.h5', 
...
if __name__ == '__main__':
    ...
    kernel = GPy.kern.RatQuad(input_dim=6, ARD=True)
        ...
        if i > 1:
            ...
            while iteration_accepted == False:
                try:
                    ...
                    save_bolfi(bolfi,fname='BOLFI_6D_at_'+str(150+50*i)+'.pkl')
                    inst = tmn.instance
                    inst.Gaussian_noise.variance=1e-10
                    inst.Gaussian_noise.variance.fix()
                    ...
                    if i%2 == 0:
                        ...
                        kernel.lengthscale[[5]].constrain_bounded(1e-6,1.0e-3)
                        inst.randomize()
                        inst.optimize()
                    iteration_accepted = True
                except:
