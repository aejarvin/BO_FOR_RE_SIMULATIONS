...
def run_CQ_fluid_simulation_uniform_RE(T_f=10, nAr_frac=15, walltime=5, 
    ...
    while plateau_current_ok != True:    
        ...
        while timestepok == False:
            ...
            try:
                ...
                timev = [0.0]
            f.close()
            # This section of the code checks if the simulation failed and increases time resolution if it did.
            if current == 0.0:
                nt = nt*1.2
                ds1.timestep.setNt(nt)
                if nt > 1e4:
                    break
            # If the simulation did not fail, proceed with analysis. 
            else:
                timestepok = True
            step_size = 1
            ...
            if abs(current - Ip) > 1e3 and timestepok == True:
                # Check whether there already are Nre values for
                # above and below the target value.
                if Flag_below == False or Flag_above == False:
                    if (current > Ip):
                        # Predicted current above the target value
                        # Set Nre_above as the current Nre value.
                        Nre_above = Nre
                        # Check whether we already have Nre_below
                        # setup. If not -> use multiplier.
                        # If yes -> use binary search.
                        if Flag_below == False:
                            Nre = Nre/mult
                        else:
                            Nre = 0.5*(Nre_above + Nre_below)
                        # Setup flag such that we have setup Nre_above
                        Flag_above = True
                    else:
                        # Predicted current below the target value
                        # Set Nre_below as the current Nre value.
                        Nre_below = Nre
                        # Check whether we already have Nre_above
                        # setup. If not -> use multiplier.
                        # If yes -> use binary search.
                        if Flag_above == False:
                            Nre = Nre*mult
                    ...
                    # If both Nre_above and Nre_below have been setup,
                    # use binary search.
                    if (current > Ip):
                        Nre_above = Nre
                    else:
    ...
    Tf_out = T_f
    print('Current Ip: ', current, ' Target Ip: ', Ip, ' Nre: ', Nre, ' T_f: ', Tf_out , ' nAr_frac: ', nAr_frac)
